...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		// 异步加载数据
		$.ajax({
			url: '${pageContext.request.contextPath}/styles/fb_data/inform_form.json',
			success: function(d) {
		...
		$btn_add.click(function () {
			...
			BootstrapDialog.show({
				...
				onshown: function(dialogReg) {
					$('#dataForm [name=informContent]').replaceWith($('<div id="weditor"></div>'));
							...
							var cfm = BootstrapDialog.confirm({
								...
								callback: function(isYes) {
									...
									if (isYes) {
										...
										formData['informContent'] = window.weditor.txt.html();
				...
				onshown: function(dialogReg) {
					...
					$.ajax({
						...
						success: function(d) {
							...
							weditor.txt.html(result.info.informContent)
						},
						error: function(d) {
							console.log('失败', d);
						}
							...
							var cfm = BootstrapDialog.confirm({
								...
								callback: function(isYes) {
									...
									if (isYes) {
										...
										var formData = $('#dataForm').serializeJson();
										formData['informContent'] = window.weditor.txt.html();
										formData['informId'] = getIdSelections()[0];
			...
			var cfm = BootstrapDialog.confirm({
				...
				callback: function(isYes) {
					if (isYes) {
						...
						$.ajax({
							url: '${pageContext.request.contextPath}/informNotice/deleteData',
							data : formData,
							success: function(d) {
								var result = $.parseJSON(d);
								console.log('提交', d, result.message, result.status);
